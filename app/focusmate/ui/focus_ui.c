...
/* LVGL's build globs every src/font/*.c into liblvgl.a, so font objects are
...
static fm_ui_cmd_t primary_cmd_for(int state)
{
  switch (state)
    ...
    case FM_IDLE:
      /* Nothing planned yet: a press should start a default session rather
       * than doing nothing. */
      return FM_UI_CMD_QUICKSTART;

    default:
      /* PLANNING: the plan is still being produced. */
